...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">LISTADO DE VALORACIONES</h1>
                </div>
                <div class="col-sm-6">
                                ...
                                <table id="table1" class="table table-bordered table-striped table-hover table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $valoracion->deportista->user->nombres }} {{ $valoracion->deportista->user->apellidos }}</td>
                                            <td>{{ $valoracion->cancha->nombre }}</td>
                                            <td class="text-center">{{ $valoracion->puntos }} ⭐</td>
                                            <td>{{ $valoracion->comentario }}</td>
                                            <td>{{ $valoracion->created_at->format('d/m/Y H:i') }}</td>
                                            <td class="d-flex justify-content-center">
                                                <a href="{{ route('admin.valoracion.edit', $valoracion->id) }}" class="btn-icon-circle btn-edit mr-1" title="Editar valoracion">
                                                    <i class="fas fa-edit me-1"></i>
                                                </a>
                                                <form action="{{ route('admin.valoracion.destroy', $valoracion->id) }}" method="POST"
                                                    id="miFormulario{{ $valoracion->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn-icon-circle btn-delete mr-1"
                                                        onclick="preguntar{{$valoracion->id}}(event)"
                                                        title="Eliminar">
                                                        <i class="fas fa-trash-alt me-1"></i>
                                                    </button>
                                                </form>
                                                <script>
                                                    function preguntar{{ $valoracion->id }}(event) {
                                                        ...
                                                        Swal.fire({
                                                            ...
                                                            if(result.isConfirmed){
                                                                document.getElementById('miFormulario{{ $valoracion->id }}').submit();
                                                            }
                                                        });
                                                    }
                                                </script>
        ...
        $(function(){
            $("#table1").DataTable({
                ...
                "language":{
                    ...
                    "info": "Mostrando _START_ a _END_ de _TOTAL_ Valoraciones",
                    "infoEmpty": "Mostrando 0 a 0 de 0 Valoraciones",
                    "infoFiltered": "(Filtrado de _MAX_ total Valoraciones)",
                    "lengthMenu": "Mostrar _MENU_ Valoraciones",
